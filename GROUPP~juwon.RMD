...
Mean, median sales by genre graph
```{r}
adu<-aggregate(BOX_total~Genre,df_edit_col_N,mean)
ggplot(adu,aes(x=Genre,y=BOX_total,fill=Genre))+ geom_bar(stat="identity",position="dodge") + theme(axis.text.x=element_text(angle=90, hjust=1)) + coord_flip()

adu1<-aggregate(BOX_total~Genre,df_edit_col_N,median)
ggplot(adu1,aes(x=Genre,y=BOX_total,fill=Genre))+ geom_bar(stat="identity",position="dodge") + theme(axis.text.x=element_text(angle=90, hjust=1)) + coord_flip()
